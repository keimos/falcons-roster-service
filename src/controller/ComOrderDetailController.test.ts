...
//Stubs
let getOrderDetailByCustomerOrderNumberStub: SinonStub;
...
describe('GET /api/v1/orders', ()=>{
    describe('Given a valid customer Order Number', ()=>{
        ...
        before((done) =>{
            ...
            //Stub
            getOrderDetailByCustomerOrderNumberStub = stub(comOrderDetailService, 'getOrderDetailByCustomerOrderNumber');
            getOrderDetailByCustomerOrderNumberStub.resolves(expectedObj);
        ...
        it('should call ComOrderDetailsService to get the details from db',() =>{
            expect(getOrderDetailByCustomerOrderNumberStub.calledOnce).to.be.true;
        })
        it('should return a body with order details',() =>{
            expect(responseBody).contains.name.match("customerOrderNumber");
        })
    })
    describe('When customer Order Number is null or empty',()=>{
        it('should return Invalid Request',() =>{
            expect(SystemError).to.throw.name.match("Invalid request");
        })
    })
}
)
